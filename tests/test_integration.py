...
class HistoricalPipelineTests(unittest.TestCase):
    ...
    def test_2026_forecast_and_backtest_run_end_to_end(self):
        ...
        # Nobody is promoted past the allowance their own market rank earns.
        allowed = promotion_allowance(
            forecast["market_rank"], DEFAULT_LEAGUE_CONFIG
        )
        self.assertTrue(
            ((forecast["market_rank"] - forecast["model_rank"]) <= allowed).all()
        )
